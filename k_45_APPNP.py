...
# Varijanta: APPNP – približna personalizovana propagacija (PageRank-style na H)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
APPNP (Approximate Personalized Propagation of Neural Predictions):
H = početne feature (Laplacian); Z = (1−α)·H + α·A_norm·Z, iterativno do fiksne tačke.
"""
...
APPNP_ALPHA = 0.1 # 0.1 alpha for APPNP
APPNP_ITERS = 20  # 20 steps for APPNP
...
def compute_appnp_embeddings(M, k=EMBED_DIM):
    ...
    H = vecs[:, :k].astype(float)
    Z = (1.0 - APPNP_ALPHA) * H.copy()
    for _ in range(APPNP_ITERS):
        Z = (1.0 - APPNP_ALPHA) * H + APPNP_ALPHA * (A_norm @ Z)
    emb = Z
    for d in range(k):
        col = emb[:, d]
...
def main():
    ...
    emb = compute_appnp_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (APPNP + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (APPNP + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 34, 37)

    Score: 0.4992675315008977
    """
...
H: početne feature = prvih k sopstvenih vektora Laplaciana (39×k).
A_norm: simetrično normalizovana A+I (D^{-1/2}(A+I)D^{-1/2}).
Propagacija: Z_0 = (1−α)·H; zatim Z_{t+1} = (1−α)·H + α·A_norm·Z_t, APPNP_ITERS=20 koraka (APPNP_ALPHA=0.1).
Embedding: Z posle iteracija; normalizacija u [0, π] (39×k).
"""
